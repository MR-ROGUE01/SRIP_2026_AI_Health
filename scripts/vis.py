...
def read_signal(filepath):
    ...
    # the timestamp uses comma for milliseconds, replacing with dot for parsing
...
def visualize(participant_folder):
    ...
    # load all signal files and event annotations
    ...
    # 3 subplots sharing the same time axis
    fig, axes = plt.subplots(3, 1, figsize=(20, 10), sharex=True)
    fig.suptitle(f"Sleep Study - {participant_name}", fontsize=14, fontweight='bold')
    fig.subplots_adjust(left=0.08)

    # thin linewidth since we have a large number of samples
    ...
    # overlay breathing events as colored shading on all 3 plots
